...
public class ProductoController {
    ...
    public ResponseEntity<?> listarProductosReOrdenConsulta() {
        try {
            ...
            respuesta.put("producto_id", info.getId_producto());
    ...
    public ResponseEntity<?> darProductoPorCaracteristica(@RequestParam(required = false) Double precioMinimo, @RequestParam(required = false) Double precioMaximo, @RequestParam(required = false) String fechaInicio, @RequestParam(required = false) String fechaFin, @RequestParam(required = false) int idCategoria) {
        try {
            ...
            respuesta.put("codigo_barras", info.getCodigoBarras());
            respuesta.put("costo_bodega", info.getCostoBodega());
            respuesta.put("precio_venta", info.getPrecioVenta());
            respuesta.put("presentacion", info.getPresentacion());
            respuesta.put("cantidad_presentacion", info.getCantidadPresentacion());
            respuesta.put("unidad_medida", info.getUnidadMedida());
            respuesta.put("volumen_empaque", info.getVolumenEmpaque());
            respuesta.put("peso_empaque", info.getPesoEmpaque());
            respuesta.put("fecha_expiracion", info.getFechaExpiracion());
            respuesta.put("categoria_id", info.getId_categoria());
            return ResponseEntity.ok(respuesta);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
